...
static void disas_set_da_iss(DisasContext *s, MemOp memop, ISSInfo issinfo)
{
     ...
     */
    syn = syn_data_abort_with_iss(0, sas, sse, srt, 0, is_acqrel, 0, 0, 0, 0, 0, is_16bit);
...
static inline void gen_bx_excret(DisasContext *s, TCGv_i32 var)
{
    ...
    if(arm_dc_feature(s, ARM_FEATURE_M_SECURITY) || (s->v7m_handler_mode && arm_dc_feature(s, ARM_FEATURE_M))) {
...
{
    static const MemOp mop_align[] = { 0, MO_ALIGN_2, MO_ALIGN_4, MO_ALIGN_8, MO_ALIGN_16,
                                       /*
...
{
    gen_helper_exception_with_syndrome_el(cpu_env, tcg_constant_i32(excp), tcg_constant_i32(syndrome), tcg_el);
}
...
{
    gen_helper_exception_with_syndrome(cpu_env, tcg_constant_i32(excp), tcg_constant_i32(syndrome));
}

static void gen_exception_insn_el_v(DisasContext *s, uint64_t pc, int excp, uint32_t syn, TCGv_i32 tcg_el)
{
    if(s->aarch64) {
...
static inline void gen_hlt(DisasContext *s, int imm)
{
     ...
     */
    if(semihosting_enabled(s->current_el != 0) && (imm == (s->thumb ? 0x3c : 0xf000))) {
...
static inline int gen_iwmmxt_address(DisasContext *s, uint32_t insn, TCGv_i32 dest)
{
    int rd;
    uint32_t offset;
    TCGv_i32 tmp;
...
static int disas_iwmmxt_insn(DisasContext *s, uint32_t insn)
{
    ...
    switch(((insn >> 12) & 0xf00) | ((insn >> 4) & 0xff)) {
        ...
        case 0x702:
            ...
            gen_op_iwmmxt_movq_M0_wRn(rd0);
            iwmmxt_load_reg(cpu_V1, rd1);
            gen_helper_iwmmxt_align(cpu_M0, cpu_M0, cpu_V1, tcg_constant_i32((insn >> 20) & 3));
            gen_op_iwmmxt_movq_wRn_M0(wrd);
            gen_op_iwmmxt_set_mup();
            break;
        case 0x01a:  //  WSUB
    ...
    switch(*tgtmode) {
        case ARM_CPU_MODE_MON:
            ...
            if(s->current_el == 1) {
                ...
                gen_exception_insn_el_v(s, s->pc_curr, EXCP_UDEF, syn_uncategorized(), tcg_el);
        ...
        case ARM_CPU_MODE_HYP:
             ...
             */
            if(!arm_dc_feature(s, ARM_FEATURE_EL2) || s->current_el < 2 || (s->current_el < 3 && *regno != 17 && *regno != 16)) {
                goto undef;
            }
            break;
        default:
...
static void gen_msr_banked(DisasContext *s, int r, int sysm, int rn)
{
    ...
    gen_helper_msr_banked(cpu_env, tcg_reg, tcg_constant_i32(tgtmode), tcg_constant_i32(regno));
...
static void gen_mrs_banked(DisasContext *s, int r, int sysm, int rn)
{
    ...
    gen_helper_mrs_banked(tcg_reg, cpu_env, tcg_constant_i32(tgtmode), tcg_constant_i32(regno));
...
#define GEN_CMP0(NAME, COND)                                                                                             \
    ...
    void gen_gvec_##NAME##0(unsigned vece, uint32_t d, uint32_t m, uint32_t opr_sz, uint32_t max_sz)                     \
    {                                                                                                                    \
        const GVecGen2 op[4] = {                                                                                         \
            { .fno = gen_helper_gvec_##NAME##0_b, .fniv = gen_##NAME##0_vec, .opt_opc = vecop_list_cmp, .vece = MO_8 },  \
            { .fno = gen_helper_gvec_##NAME##0_h, .fniv = gen_##NAME##0_vec, .opt_opc = vecop_list_cmp, .vece = MO_16 }, \
            { .fni4 = gen_##NAME##0_i32, .fniv = gen_##NAME##0_vec, .opt_opc = vecop_list_cmp, .vece = MO_32 },          \
...
void gen_gvec_srshr(unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs, int64_t shift, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    static const GVecGen2i ops[4] = {
        { .fni8 = gen_srshr8_i64, .fniv = gen_srshr_vec, .fno = gen_helper_gvec_srshr_b, .opt_opc = vecop_list, .vece = MO_8 },
        { .fni8 = gen_srshr16_i64, .fniv = gen_srshr_vec, .fno = gen_helper_gvec_srshr_h, .opt_opc = vecop_list, .vece = MO_16 },
        { .fni4 = gen_srshr32_i32, .fniv = gen_srshr_vec, .fno = gen_helper_gvec_srshr_s, .opt_opc = vecop_list, .vece = MO_32 },
...
void gen_gvec_urshr(unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs, int64_t shift, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = { INDEX_op_shri_vec, INDEX_op_add_vec, 0 };
    static const GVecGen2i ops[4] = {
        { .fni8 = gen_urshr8_i64, .fniv = gen_urshr_vec, .fno = gen_helper_gvec_urshr_b, .opt_opc = vecop_list, .vece = MO_8 },
        { .fni8 = gen_urshr16_i64, .fniv = gen_urshr_vec, .fno = gen_helper_gvec_urshr_h, .opt_opc = vecop_list, .vece = MO_16 },
        { .fni4 = gen_urshr32_i32, .fniv = gen_urshr_vec, .fno = gen_helper_gvec_urshr_s, .opt_opc = vecop_list, .vece = MO_32 },
...
{
    static const TCGOpcode vecop_list[] = { INDEX_op_neg_vec, INDEX_op_shlv_vec, INDEX_op_shrv_vec, INDEX_op_cmp_vec, 0 };
    static const GVecGen3 ops[4] = {
...
void gen_gvec_sshl(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    static const TCGOpcode vecop_list[] = {
        INDEX_op_neg_vec, INDEX_op_umin_vec, INDEX_op_shlv_vec, INDEX_op_sarv_vec, INDEX_op_cmp_vec, INDEX_op_cmpsel_vec, 0
    };
    static const GVecGen3 ops[4] = {
...
static void gen_uqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec sat, TCGv_vec a, TCGv_vec b)
{
...
{
    static const TCGOpcode vecop_list[] = { INDEX_op_usadd_vec, INDEX_op_cmp_vec, INDEX_op_add_vec, 0 };
    static const GVecGen4 ops[4] = {
        { .fniv = gen_uqadd_vec, .fno = gen_helper_gvec_uqadd_b, .write_aofs = true, .opt_opc = vecop_list, .vece = MO_8  },
        { .fniv = gen_uqadd_vec, .fno = gen_helper_gvec_uqadd_h, .write_aofs = true, .opt_opc = vecop_list, .vece = MO_16 },
        { .fniv = gen_uqadd_vec, .fno = gen_helper_gvec_uqadd_s, .write_aofs = true, .opt_opc = vecop_list, .vece = MO_32 },
        { .fniv = gen_uqadd_vec, .fno = gen_helper_gvec_uqadd_d, .write_aofs = true, .opt_opc = vecop_list, .vece = MO_64 },
    };
    tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}

static void gen_sqadd_vec(unsigned vece, TCGv_vec t, TCGv_vec sat, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
    tcg_gen_add_vec(vece, x, a, b);
...
{
    static const TCGOpcode vecop_list[] = { INDEX_op_ssadd_vec, INDEX_op_cmp_vec, INDEX_op_add_vec, 0 };
    static const GVecGen4 ops[4] = {
        { .fniv = gen_sqadd_vec, .fno = gen_helper_gvec_sqadd_b, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_8  },
        { .fniv = gen_sqadd_vec, .fno = gen_helper_gvec_sqadd_h, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_16 },
        { .fniv = gen_sqadd_vec, .fno = gen_helper_gvec_sqadd_s, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_32 },
        { .fniv = gen_sqadd_vec, .fno = gen_helper_gvec_sqadd_d, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_64 },
    };
    tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}

static void gen_uqsub_vec(unsigned vece, TCGv_vec t, TCGv_vec sat, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
...
{
    static const TCGOpcode vecop_list[] = { INDEX_op_ussub_vec, INDEX_op_cmp_vec, INDEX_op_sub_vec, 0 };
    static const GVecGen4 ops[4] = {
        { .fniv = gen_uqsub_vec, .fno = gen_helper_gvec_uqsub_b, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_8  },
        { .fniv = gen_uqsub_vec, .fno = gen_helper_gvec_uqsub_h, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_16 },
        { .fniv = gen_uqsub_vec, .fno = gen_helper_gvec_uqsub_s, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_32 },
        { .fniv = gen_uqsub_vec, .fno = gen_helper_gvec_uqsub_d, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_64 },
    };
    tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}

static void gen_sqsub_vec(unsigned vece, TCGv_vec t, TCGv_vec sat, TCGv_vec a, TCGv_vec b)
{
    TCGv_vec x = tcg_temp_new_vec_matching(t);
    tcg_gen_sub_vec(vece, x, a, b);
...
{
    static const TCGOpcode vecop_list[] = { INDEX_op_sssub_vec, INDEX_op_cmp_vec, INDEX_op_sub_vec, 0 };
    static const GVecGen4 ops[4] = {
        { .fniv = gen_sqsub_vec, .fno = gen_helper_gvec_sqsub_b, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_8  },
        { .fniv = gen_sqsub_vec, .fno = gen_helper_gvec_sqsub_h, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_16 },
        { .fniv = gen_sqsub_vec, .fno = gen_helper_gvec_sqsub_s, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_32 },
        { .fniv = gen_sqsub_vec, .fno = gen_helper_gvec_sqsub_d, .opt_opc = vecop_list, .write_aofs = true, .vece = MO_64 },
    };
    tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz, max_sz, &ops[vece]);
}
...
void gen_gvec_sabd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    static const GVecGen3 ops[4] = {
        ...
        { .fni4 = gen_sabd_i32, .fniv = gen_sabd_vec, .fno = gen_helper_gvec_sabd_s, .opt_opc = vecop_list, .vece = MO_32 },
        { .fni8 = gen_sabd_i64,
         .fniv = gen_sabd_vec,
...
void gen_gvec_uabd(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs, uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
    ...
    static const GVecGen3 ops[4] = {
        ...
        { .fni4 = gen_uabd_i32, .fniv = gen_uabd_vec, .fno = gen_helper_gvec_uabd_s, .opt_opc = vecop_list, .vece = MO_32 },
        { .fni8 = gen_uabd_i64,
         .fniv = gen_uabd_vec,
...
static void do_coproc_insn(DisasContext *s, int cpnum, int is64, int opc1, int crn, int crm, int opc2, bool isread, int rt,
                           int rt2)
{
    ...
    if(ri) {
        ...
        if(s->hstr_active || ri->accessfn || (arm_dc_feature(s, ARM_FEATURE_XSCALE) && cpnum < 14)) {
        ...
        if(isread) {
            ...
            if(is64) {
                ...
                if(ri->type & ARM_CP_CONST) {
                    ...
                    gen_helper_get_cp_reg64(tmp64, cpu_env, tcg_constant_ptr(ri));
            ...
            if(is64) {
                ...
                if(ri->writefn) {
                    gen_helper_set_cp_reg64(cpu_env, tcg_constant_ptr((int64_t)ri), tmp64);
                } else if(ri->fieldoffset != 0) {
        ...
        need_exit_tb = ((tb_cflags(s->base.tb) & CF_USE_ICOUNT) && (ri->type & ARM_CP_IO));
...
static void gen_srs(DisasContext *s, uint32_t mode, uint32_t amode, bool writeback)
{
    ...
    if(s->current_el == 1 && !s->ns && mode == ARM_CPU_MODE_MON) {
        gen_exception_insn_el(s, s->pc_curr, EXCP_UDEF, syn_uncategorized(), 3);
        return;
    }
...
static bool op_s_rxr_shi(DisasContext *s, arg_s_rrr_shi *a, void (*gen)(TCGv_i32, TCGv_i32), int logic_cc, StoreRegKind kind)
{
    TCGv_i32 tmp;
...
static bool op_s_rxr_shr(DisasContext *s, arg_s_rrr_shr *a, void (*gen)(TCGv_i32, TCGv_i32), int logic_cc, StoreRegKind kind)
{
    TCGv_i32 tmp1, tmp2;
...
static bool op_s_rxi_rot(DisasContext *s, arg_s_rri_rot *a, void (*gen)(TCGv_i32, TCGv_i32), int logic_cc, StoreRegKind kind)
{
    TCGv_i32 tmp;
    uint32_t imm;
 ...
 */
static bool do_mve_shl_ri(DisasContext *s, arg_mve_shl_ri *a, WideShiftImmFn *fn)
{
    TCGv_i64 rda;
    ...
    if(!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) || a->rdahi == 13) {
...
static bool do_mve_shl_rr(DisasContext *s, arg_mve_shl_rr *a, WideShiftFn *fn)
{
    ...
    if(!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) || a->rdahi == 13 || a->rm == 13 || a->rm == 15 ||
...
static bool do_mve_sh_ri(DisasContext *s, arg_mve_sh_ri *a, ShiftImmFn *fn)
{
    ...
    if(!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) || a->rda == 13 || a->rda == 15) {
...
static bool do_mve_sh_rr(DisasContext *s, arg_mve_sh_rr *a, ShiftFn *fn)
{
    ...
    if(!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) || a->rda == 13 || a->rda == 15 || a->rm == 13 ||
       a->rm == 15 || a->rm == a->rda) {
...
static bool trans_BKPT(DisasContext *s, arg_BKPT *a)
{
    ...
    s->eci_handled = true;
    if(arm_dc_feature(s, ARM_FEATURE_M) && semihosting_enabled(s->current_el == 0) && (a->imm == 0xab)) {
        gen_exception_internal_insn(s, s->pc_curr, EXCP_SEMIHOST);
    } else {
        gen_set_pc_im(s->pc_curr);
...
static void op_addr_rr_post(DisasContext *s, arg_ldst_rr *a, TCGv_i32 addr, int address_offset)
{
    if(!a->p) {
        TCGv_i32 ofs = load_reg(s, a->rm);
        gen_arm_shift_im(ofs, a->shtype, a->shimm, 0);
...
static void op_addr_ri_post(DisasContext *s, arg_ldst_ri *a, TCGv_i32 addr, int address_offset)
{
    if(!a->p) {
        if(a->u) {
...
static bool op_div(DisasContext *s, arg_rrr *a, bool u)
{
    ...
    if(s->thumb ? !dc_isar_feature(aa32_thumb_div, s) : !dc_isar_feature(aa32_arm_div, s)) {
        return false;
    }
...
static bool trans_WLS(DisasContext *s, arg_WLS *a)
{
    ...
    if(a->size != 4) {
         ...
         */
        if(s->fp_excp_el) {
            gen_exception_insn_el(s, s->pc_curr, EXCP_NOCP, syn_uncategorized(), s->fp_excp_el);
            return true;
        }
    }
...
static bool trans_VCTP(DisasContext *s, arg_VCTP *a)
{
    ...
    tcg_gen_movcond_i32(TCG_COND_LEU, masklen, masklen, tcg_constant_i32(1 << (4 - a->size)), rn_shifted, tcg_constant_i32(16));
...
static bool trans_CBZ(DisasContext *s, arg_CBZ *a)
{
    ...
    arm_gen_condlabel(s);
    tcg_gen_brcondi_i32(a->nz ? TCG_COND_EQ : TCG_COND_NE, tmp, 0, s->condlabel);
    tcg_temp_free_i32(tmp);
    gen_jmp(s, read_pc(s) + a->imm);
    return true;
}
...
static bool trans_SVC(DisasContext *s, arg_SVC *a)
{
    ...
    if(!arm_dc_feature(s, ARM_FEATURE_M) && semihosting_enabled(s->current_el == 0) && (a->imm == semihost_imm)) {
        gen_exception_internal_insn(s, s->pc_curr, EXCP_SEMIHOST);
    } else {
        gen_set_pc_im(s->base.pc_next);
        s->svc_imm = a->imm;
...
{
    if(s->thumb ? !ENABLE_ARCH_7 && !arm_dc_feature(s, ARM_FEATURE_M) : !ENABLE_ARCH_6K) {
        return false;
    }
    gen_clrex(s);
    return true;
...
static void disas_arm_insn(DisasContext *s, unsigned int insn)
{
    ...
    if(arm_dc_feature(s, ARM_FEATURE_XSCALE)) {
        if(((insn & 0x0c000e00) == 0x0c000000) && ((insn & 0x03000000) != 0x03000000)) {
...
static bool thumb_insn_is_16bit(DisasContext *s, uint32_t pc, uint32_t insn)
{
     ...
     */
    if(arm_dc_feature(s, ARM_FEATURE_THUMB2) || arm_dc_feature(s, ARM_FEATURE_M)) {
...
void disas_thumb2_insn(DisasContext *s, uint32_t insn)
{
    ...
    if(arm_dc_feature(s, ARM_FEATURE_M) && !arm_dc_feature(s, ARM_FEATURE_V7)) {
        int i;
        bool found = false;
        static const uint32_t armv6m_insn[] = { 0xf3808000 /* msr */, 0xf3b08040 /* dsb */, 0xf3b08050 /* dmb */,
                                                0xf3b08060 /* isb */, 0xf3e08000 /* mrs */, 0xf000d000 /* bl */ };
        static const uint32_t armv6m_mask[] = { 0xffe0d000, 0xfff0d0f0, 0xfff0d0f0, 0xfff0d0f0, 0xffe0d000, 0xf800d000 };
    ...
    if(disas_t32(s, insn) || disas_vfp_uncond(s, insn) || disas_neon_shared(s, insn) ||
       (cpu_isar_feature(aa32_mve, &cpu->arm_core_config) && disas_mve(s, insn)) || ((insn >> 28) == 0xe && disas_vfp(s, insn))) {
        return;
    }
...
void arm_tr_init_disas_context(DisasContextBase *dcbase, CPUState *env)
{
    ...
    if(arm_feature(env, ARM_FEATURE_M)) {
        ...
        dc->v8m_secure = arm_feature(env, ARM_FEATURE_M_SECURITY) && regime_is_secure(env, dc->mmu_idx);
...
static bool thumb_insn_is_unconditional(DisasContext *s, uint32_t insn)
{
    ...
    if((insn & 0xffffffc0) == 0xba80 && arm_dc_feature(s, ARM_FEATURE_V8) && !arm_dc_feature(s, ARM_FEATURE_M)) {
    ...
    if(insn == 0xe97fe97f && arm_dc_feature(s, ARM_FEATURE_V8) && arm_dc_feature(s, ARM_FEATURE_M)) {
        /* SG: v8M only */
        return true;
    }
    ...
    if(dc->base.is_jmp == DISAS_BX_EXCRET) {
        ...
        switch(dc->base.is_jmp) {
            ...
            case DISAS_NORETURN:
                ...
                break;
            case DISAS_WFI:
                gen_helper_wfi(cpu_env, tcg_constant_i32(dc->base.pc_next - dc->pc_curr));
                /*
